  ...
  useEffect(() => {
    const fetchUniversityEntities = async () => {
      try {
        ...
        console.error('Error getting list of campus: ', error);
        setError('Error getting list of campus');
      }
    };
    ...
    <Container className={classes.container} style={{ marginTop: '20px' }}>
      ...
      {universityEntities?.length > 0 ? (
          ...
          {universityEntities?.map((entity) => (
            <li key={entity.id}>
              <Typography variant="body1">
                {entity.name.value} -{' '}
                <RouterLink to={`/entities/campus/${entity.id}`}>View Campus</RouterLink>
              </Typography>
            </li>
          ))}
        </ul>
        ...
        <Paper className={classes.notlist}>
          <Typography>There are no registered campus</Typography>
        </Paper>
      )}
